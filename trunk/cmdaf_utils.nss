...
struct cmd_data_str cmdSetDataStructure(string sSpeech = CMD_EMPTY_SPEECH, string sCommand = CMD_EMPTY_COMMAND_DATAS, int iOpeningTokenPosition = TOKEN_POSITION_ERROR, int iClosingTokenPosition = TOKEN_POSITION_ERROR);
...
int cmdIsParameterDefined(string sCommand, string sName) {
    return (FindSubString(sCommand, CMD_PARAMETER_TOKEN+sName) != TOKEN_POSITION_ERROR);
}
