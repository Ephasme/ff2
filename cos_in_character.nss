...
const string PCWP_VARNAME = "chardatawp";
const string PCWP_TAG = "chardatawp";
const string PCWP_RESREF = "chardatawp";

// Waypoint qui permet d'envoyer le PJ au centre de formation.
const string WP_START_TRAINING_CENTER = "wp_start_training_center";
...
object cosGetPCWaypoint(object oPC) {
    ...
    if (!GetIsObjectValid(oPCWP)) {
        oPCWP = CreateObject(OBJECT_TYPE_WAYPOINT, PCWP_RESREF, GetStartingLocation(), FALSE, PCWP_TAG);
        if (GetIsObjectValid(oPCWP)) {
            SetLocalObject(oPC, PCWP_VARNAME, oPCWP);
        }
    }
    return oPCWP;
}
...
void cosJumpPCToTrainingCenter(object oPC) {
    pv_cosJumpPC(oPC, GetLocation(GetWaypointByTag(WP_START_TRAINING_CENTER)));
}
